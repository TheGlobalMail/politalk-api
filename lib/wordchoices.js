...
exports.forTerm = function(term, fullMatch, cb){
  ...
  if (_.isEmpty(tokens) || _.isEmpty(tokens[0]) || tokens[0].length === 1){
    results.message = "must start with a word with two or more characters";
    return cb(null, results);
  }

  if (!notIgnoring(tokens[0])){
    results.message =  "starts with a word that occurs too often. Try starting your search with another word";
    return cb(null, results);
  }
