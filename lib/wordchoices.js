...
exports.forTerm = function(term, options, cb){
  // max three words
  var results;
  var tokens = _.compact(tokenizer.tokenize(term)).slice(0, 3);
  var joinOnHansards = false;
  var field = 'word';
  ...
  if (options.party){
    conditions.push('party = \'' + options.party + '\'');
  }

  if (options.speaker_id){
    joinOnHansards = true;
    conditions.push('speaker_id = \'' + options.speaker_id + '\'');
  }

  if (options.from && options.to){
    if (joinOnHansards){
      conditions.push('hansards.date between \'' + options.from + '-01-01\' and \'' + options.to + '-12-31\'');
    }else{
      conditions.push('date between \'' + options.from + '-01-01\' and \'' + options.to + '-12-31\'');
    }
  }
  ...
  wordchoices.checkNotTooMany(conditions, tokens, joinOnHansards, function(err, tooMany){ 
    ...
    var table = lookupClusterTable(tokens[0]);
    var sql = "select " + table + ".party, week, " +
      "count(word1) as freq, string_agg(hansard_id, ',') as ids " + 
      " from " +
      table +
      (joinOnHansards ? 'inner join hansards on id = hansard_id ' : '') +
      "where " + 
      conditions.join(' and ') +
    ...
    db.readonlyQuery(sql, tokens, function(err, result){
      ...
      _.each(results.data, function(datum){
        datum.freq = parseInt(datum.freq, 10);
      });
      cb(null, results);
    });
...
exports.forTermByYear = function(term, exactMatch, party, cb){
  exports.forTerm(term, {exactMatch: exactMatch}, function(err, wordChoices){
    var forTermByYear = {};
...
exports.checkNotTooMany = function(conditions, tokens, joinOnHansards, cb){
  var max = 43000;
  ...
  return es.map(function(data, cb){
    hansard.byId(data.id, function(err, hansardRecord){
      ...
      if (process.env.VERBOSE){
        ...
        console.error("sample: " + util.inspect(indexes.slice(0, 8)));
      }
