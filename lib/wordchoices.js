...
var TOO_FREQUENCY = 20000;

exports.forTerm = function(term, cb){

  exports.tooMany(term, function(err, count){
    if (count > TOO_FREQUENCY){
      return cb("Over " + count + " speeches matched. Choose a less used term");
    }
    var query = "select date, party, sum(CountInString(html, $1)) as freq, " +
...
exports.tooMany = function(term, cb){
  var query = "select count(id) as count " +
    "from hansards " +
    "where searchable @@ to_tsquery('''" + term +  "''')";
