...
exports.forTerm = function(term, fullMatch, cb){
  ...
  if (_.isEmpty(tokens) || _.isEmpty(tokens[0]) || tokens[0].length === 1){
    invalid =  "The first word of the phrase " + term + " must contain two or more characters";
    return cb(null, {invalid: true, message: invalid, data : []});
  }

  if (!notIgnoring(tokens[0])){
    invalid =  "The first word '" + tokens[0] + "' is too common. Try starting your search with another word";
    return cb(null, {invalid: true, firstwordtoocommon: true, message: invalid, data : []});
  }
