...
exports.forTerm = function(term, fullMatch, cb){
  console.error("got term: " + term);
  var tokens = _.compact(tokenizer.tokenize(term));
  var field = 'word';

  console.error("Got fullMatch: " + fullMatch + ' and ' + !!fullMatch);
  ...
  var sql = "select party, week, " +
    ...
    _.map(tokens, function(t, i){ return field + (i+1) + " = $" + (i+1); }).join(" and ") +
    " group by 1,2 order by 1,2";
  console.error(sql + " with " + tokens.join(','));
