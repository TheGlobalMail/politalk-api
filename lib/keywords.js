    ...
    async.forEachSeries(rows, function(row, next){
      wordchoices.forTerm(row.text, search, function(err, termInfo){
        ...
        if (row.text.split(' ').length < 4){
          ...
          if (process.env.VERBOSE) console.error('corrected ' + row.text + ' from ' + row.score + ' to ' + row.wordchoices + ' and used ' + row.frequency);
        }
        next();
      });
    }, function(err){
...
exports.generateYearList = function(years, cb){
  if (process.env.VERBOSE) console.error('Generating year lists');
  async.forEachSeries(years, function(yearRange, next){
    ...
    if (process.env.VERBOSE) console.error('doing range: ' + yearRange);
...
exports.generatePartyList = function(years, cb){
  if (process.env.VERBOSE) console.error('Generating party lists');
...
exports.generateSpeakerList = function(years, cb){
  exports.getAllSpeakers(function(err, result){
    ...
    if (process.env.VERBOSE) console.error('Generating speaker lists');
