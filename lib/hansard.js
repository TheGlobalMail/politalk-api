...
exports.filterDownloadedStream = require('./hansard/filter-downloaded-stream');
...
exports.lastSpeechDatesForHouses = function(cb){
  var housesToQuery = [{name: 'representatives', query: 'house'}, {name: 'senate', query: 'senate'}];
  async.map(housesToQuery, function(house, next){
    var query = 'select date from hansards order by date desc where id like \'' + house.query + '%\' limit 1';
    db.query(query, function(err, result){
      var date = result && result.rows[0] && result.rows[0].date;
      if (date) date.addDays(1);
      next(err, { name: house.name, date: date });
    });
  }, cb);
};
...
exports.extractKeywordsWithFrequency1 = function(speechId, html, date, cb){
  var results = _.select(gramophone.extract(html, gramophoneOptions), function(result){
    return result.tf === 1;
  });
  async.forEach(results, function(result, done){
    var stem = _.map(result.term.split(' '), natural.PorterStemmer.stem).join(" ");
...
exports.createStream = function(ids, cb){
  var limit = process.env.HANSARD_LIMIT || 50;
  var sql = 'select member.*, speaker,html,date,time,time_of_day,id from hansards ' +
    'inner join member on member.member_id = hansards.speaker_id ' +
    'where id in (' + _.map(ids.split(','), function(id){ return "'" + sanitiseId(id) + "'"; }) + ') order by date, time limit $1';
  db.createStream(sql, [limit], cb);
};
