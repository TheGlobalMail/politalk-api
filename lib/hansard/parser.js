...
Parser.prototype.needToCalculateExtraMetadata = function(sections) {
  return _.detect(sections, function(section){
    return section.type === 'speech' && !section.duration;
  });
};
...
Parser.prototype.calculateExtraMetadata = function(sections) {
  ...
  _.each(sections, function(section, index){
    ...
    if (currentSpeech && section.type === 'speech' && section.speaker_id === currentSpeech.speaker_id){
      ...
      if (currentSpeech){
        duration = moment.duration(getTime(section) - getTime(currentSpeech)).asMinutes();
      ...
      if (adjournment){
        section.duration = moment.duration(getTime(section, adjournment[1] + ':' + adjournment[2]) - getTime(section)).asMinutes();
  ...
  if (currentSpeech){
    sections[currentSpeechIndex].duration = 0;
    sections[currentSpeechIndex].talktype = 'speech';
  }
  return sections;
};
...
Parser.prototype.save = function(sections, cb) {
  async.forEachSeries(sections, function(section, done){
    if (section.type === 'major-heading'){
      Hansard.addHeading(section, done);
    }else if (section.type === 'minor-heading'){
      Hansard.addSubHeading(section, done);
    }else if (section.type === 'speech'){
      Hansard.addSpeech(section, done);
...
function debug(sections){
  _.each(sections, function(section){
    if (section.type === 'speech'){
      console.error(section.timeOfDay + ' | ' + section.speaker + ' (' + section.talktype + '): ' + section.duration + ' (words = ' + section.words + ')');
    }else{
      console.error(section.type);
    }
  });
}
